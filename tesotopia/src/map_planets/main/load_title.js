	...
	initBuffers() {
        ...
        for (var latNumber = 0; latNumber <= latitudeBands; latNumber++) {
            ...
            for (var longNumber = 0; longNumber <= longitudeBands; longNumber++) {
                ...
                var x = (radius) * cosPhi * sinTheta + this.pos.x;
                var y = (radius) * cosTheta + this.pos.y;
                var z = (radius) * sinPhi * sinTheta + this.pos.z;
...
class gamePlanet {
    constructor(id,name,r,pos) {
        this.id = id;
        this.name = name;                       /// Наименование игровой планеты
        this.r = r;                             /// Радиус планеты
        this.pos = pos;                         /// Положение в пространстве
        this.vertexBuffer = null;               /// Буфер вершин планеты
        this.indexBuffer = null;                /// Буфер индексов планеты
        this.textureCoordsBuffer = null;        /// Буфер координат текстуры планеты
        this.planetTexture = null;              /// Текстура планеты
        this.image = null;

        if (id !== 0) {
            this.prevObject = new previewObject(name,0.99 * r,pos);            
            this.isPrev = false;     
            this.colorPicked = [id, id, id,id];
        }
    }
    ...
    setTexture(url) {
        ...
        if (this.id !== 0) {
            this.prevObject.setTexture(url);
        }
    }

    isPicked(xMouse, yMouse) {
    	let u_Picked = gl.getUniformLocation(shaderProgram, 'u_Picked');
    	let u_colorPick = gl.getUniformLocation(shaderProgram, 'u_colorPick');
		
	    gl.uniform1f(u_Picked, 1.0);
        gl.uniform4fv(u_colorPick, [this.id/255, this.id/255, this.id/255, this.id/255]); 
        
        this.prevObject.draw();
        
        let pixels = new Uint8Array(4);
        let pixelsMainModel = new Uint8Array(this.colorPicked);
        gl.readPixels(xMouse, yMouse, 1, 1, gl.RGBA, gl.UNSIGNED_BYTE, pixels);
        gl.uniform1f(u_Picked, 0.0);
    ...
    initBuffers() {
        ...
        for (var latNumber = 0; latNumber <= latitudeBands; latNumber++) {
            ...
            for (var longNumber = 0; longNumber <= longitudeBands; longNumber++) {
                ...
                var x = (radius + scale) * cosPhi * sinTheta + this.pos.x;
                var y = (radius + scale) * cosTheta + this.pos.y;
                var z = (radius + scale) * sinPhi * sinTheta + this.pos.z;
    ...
    draw() {
        ...
        if (this.isPrev) {
    		gl.uniform3fv(u_Pick_ok, [0.5, 0.5, 0.5]);
    	} else {
...
var object = [
     new gamePlanet(0,"background",  75.0, positions[0]),
     new gamePlanet(1,"Alpha_7GM",   2.1,  positions[1]), 
     new gamePlanet(2,"Beta_3PR_FS", 2.9,  positions[2]),
     new gamePlanet(3,"Gamma_2TT",   2.8,  positions[3]),
     new gamePlanet(4,"Theta_345",   2.7,  positions[4]),
     new gamePlanet(5,"Aplha_65TM",  2.5,  positions[5])
];
...
function handleMouseMove(event) {
    ...
    if (!mouseDown) {
        ...
        for (let i=1; i < object.length; i++) {
            if (object[i].isPicked(curX,curY)) {
                if (!object[i].isPrev) {
                    object[i].r += 0.1;
                    object[i].isPrev = true;
                }
            } else if (object[i].isPrev) {
                object[i].r -=0.1;
                object[i].isPrev = false;
            }
        }
